...
1. **镜像构建**：`.github/workflows/fnos-build-fpk.yml`（模板，`workflow_dispatch` 手动触发，默认不在上游仓库自动运行）的 `image` job 用 `fnos/Dockerfile` 从仓库源码构建 Octop 镜像并推送到 `ghcr.io/jubaoliang/octop:latest`（含全部附加组件）。
2. **安装包构建**：同一 workflow 的 `fpk` job 用 `scripts/build-fpk.sh` 把 `fnos/` 打成 Docker 版 `.fpk`；`native` job 从当前仓库源码构建 wheel、用 uv 将 Octop 核心及依赖安装到 `fnos-native/app/site-packages`（不捆绑 Python 运行时，依赖飞牛应用商店 Python 3.12），再打成本地版 `.fpk`（`continue-on-error`，失败不阻塞 Docker 版）。两者均以滚动发布 `fnos-vanilla-latest` 提供下载。
